...
import { type NoteItem, type NoteTag } from '../data/mockNotesData'
...
const toNoteItem = (note: NoteResponse): NoteItem => ({
  id: String(note.id),
  title: note.title,
  ...
  suggestions: [
    ...
    ...note.content.statutes.map((item) => `建议结合条文复习：${item.slice(0, 36)}${item.length > 36 ? '…' : ''}`),
  ].slice(0, 3),
})
